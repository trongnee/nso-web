...
(function () {
  toastr.options = window.config.toastr;
  setTimeout(function () {
...
(function () {
  "undefined" != typeof numeral && numeral.locale("vi");
  ...
  const isLogin = document.body.id === "login-page";
  if (!isLogin) {
    const searchMenu = function (data) {
      return function (searchTerm, callback) {
        let filteredResults = [];
        data.forEach((item) => {
          const itemName = item.label.toLowerCase();
          const searchTermLower = searchTerm.toLowerCase();
          // Kiểm tra nếu tên bắt đầu với từ khóa tìm kiếm
          if (itemName.startsWith(searchTermLower)) {
            filteredResults.push(item);
          } else {
            // Kiểm tra nếu tên không bao gồm từ khóa tìm kiếm
            if (!itemName.includes(searchTermLower)) {
              return [];
            }
            filteredResults.push(item);
            filteredResults.sort((a, b) => (b.label < a.label ? -1 : 1));
          }
        });
        // Gọi callback với danh sách kết quả
        callback(filteredResults);
      };
    };
    const navSearch = async () => {
      const search = await $.ajax({
        url: "/admin/search",
        type: "GET",
      });
      ...
      searchInput
        .typeahead(
          ...
          },
          {
            ...
            display: "name",
            limit: 4,
            source: searchMenu(search.menu),
            templates: {
              header:
                '<h6 class="suggestions-header text-primary mb-0 mx-4 mt-3 pb-2">Các trang</h6>',
              suggestion: function ({ label, icon, url }) {
                return `<a href="${url}">
                                        <div>
              ...
              notFound: `<div class="not-found px-4 py-2">
                                        <h6 class="suggestions-header text-primary mb-2">Các trang</h6>
                                        <p class="py-2 mb-0">
                                            <i class="ti ti-alert-circle ti-xs me-2"></i>Không có dữ liệu.
                                        </p>
                                    </div>`,
            },
          }
          ...
          //     source: t(a.members),
          //     templates: {
          //         header: '<h6 class="suggestions-header text-primary mb-0 mx-4 mt-3 pb-2">Tài khoản</h6>',
          ...
          //         },
          //         notFound:
          //             '<div class="not-found px-4 py-2"><h6 class="suggestions-header text-primary mb-2">Tài khoản</h6><p class="py-2 mb-0"><i class="ti ti-alert-circle ti-xs me-2"></i>Không có dữ liệu</p></div>',
          //     },
          // }
        )
        .bind("typeahead:render", function () {
          backDrop.addClass("show").removeClass("fade");
        })
        .bind("typeahead:close", function () {
          searchInput.val(""),
            searchInput.typeahead("val", ""),
            searchWrapper.addClass("d-none"),
            backDrop.addClass("fade").removeClass("show");
        });

      let scrollbar;
      $(".navbar-search-suggestion").each(function () {
        scrollbar = new PerfectScrollbar($(this)[0], {
          wheelPropagation: !1,
          suppressScrollX: !0,
        });
      });
